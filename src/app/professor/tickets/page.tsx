...
export default function ProfessorTicketsPage() {
  const [tickets, setTickets] = useState<Ticket[]>(ticketsData.filter(t => t.author === userData.professor.name));
  ...
  const handleCreateTicket = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newTicket: Ticket = {
      ...
      status: 'open',
      author: userData.professor.name, // Assuming the current user
      messages: [
        {
                ...
                <div className="space-y-6 py-4 max-h-[60vh] overflow-y-auto pr-4">
                   {selectedTicket.messages.map((msg, index) => (
                       <div key={index} className="flex flex-col gap-2">
                            ...
                            <div className={cn("p-4 rounded-lg", msg.author === userData.professor.name ? 'bg-muted/50' : 'bg-primary/10')}>
