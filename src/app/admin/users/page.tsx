...
import RoleDetailsModal from '@/components/admin/role-details-modal';
            ...
            <Card>
                ...
                <CardContent className="flex flex-col md:flex-row gap-4 justify-between">
                     ...
                     <div className="flex gap-2">
                        <Button variant="outline" asChild><Link href="/admin/users/archived"><Archive className="mr-2 h-4 w-4"/> Archives</Link></Button>
                                ...
                                return (
                                    <TableRow key={user.id} className="even:bg-muted/40">
                                        ...
                                        <TableCell className="text-right">
                                             <DropdownMenu>
                                                <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" disabled={isAdmin && users.filter(u => u.role === 'admin').length === 1}><MoreHorizontal/></Button></DropdownMenuTrigger>
                                                <DropdownMenuContent>
                                                    ...
                                                    <DropdownMenuItem asChild><Link href="/admin/users/archived"><Archive className="mr-2"/>Archiver</Link></DropdownMenuItem>
                                                    <DropdownMenuSeparator/>
...
const RolesTab = () => {
    ...
    const usersByRole = useMemo(() => {
        return usersData.reduce((acc, user) => {
            ...
            return acc;
        }, {} as Record<string, UniversityUser[]>);
    }, []);
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Visualisez et modifiez les permissions pour chaque rôle au sein de l'université.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                        ...
                        <TableBody>
                            {Object.entries(roleConfig).map(([roleKey, roleDetails]) => (
                                <TableRow key={roleKey}>
                                    <TableCell className="font-semibold">{roleDetails.label}</TableCell>
                                    <TableCell className="text-sm text-muted-foreground">{roleDetails.description}</TableCell>
                                    <TableCell>{usersByRole[roleKey]?.length || 0}</TableCell>
                                    <TableCell className="text-right">
                                        <Button variant="outline" size="sm" onClick={() => setSelectedRole({ ...roleDetails, key: roleKey, users: usersByRole[roleKey] || [] })}>
                                            <UserCog className="mr-2 h-4 w-4" /> Gérer le rôle
                                        </Button>
                                    </TableCell>
                                </TableRow>
                            ))}
            ...
            <RoleDetailsModal role={selectedRole} isOpen={!!selectedRole} onClose={() => setSelectedRole(null)} />
        </div>
    )
}
