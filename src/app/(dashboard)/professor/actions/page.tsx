...
type ActionType = 'connexion' | 'publication' | 'correction' | 'création-évaluation' | 'message' | 'rapport';
...
const actionConfig: Record<ActionType, { label: string; icon: LucideIcon; color: string; }> = {
  ...
  publication: { label: 'Publication', icon: Upload, color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' },
  correction: { label: 'Correction', icon: Edit, color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300' },
  'création-évaluation': { label: 'Évaluation', icon: ClipboardCheck, color: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300' },
  message: { label: 'Message', icon: MessageSquare, color: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300' },
  rapport: { label: 'Rapport', icon: FileBarChart, color: 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-800 dark:text-cyan-300' },
};
...
const mockActions: Action[] = [
  { id: 't-1', date: new Date(new Date(today).setHours(8, 15, 42)), type: 'connexion', details: 'Première connexion de la journée', location: 'Campus de Paris', device: 'Windows' },
  { id: 't-2', date: new Date(new Date(today).setHours(8, 30, 15)), type: 'publication', details: 'Publication du cours "SQL Avancé.pdf"', location: 'Campus de Paris', device: 'Windows' },
  { id: 't-3', date: new Date(new Date(today).setHours(9, 45, 22)), type: 'correction', details: 'Correction de 12 copies pour le TP1 - L3 Informatique', location: 'Campus de Paris', device: 'Windows' },
  ...
  { id: 'y-2', date: new Date(new Date(yesterday).setHours(9, 10, 15)), type: 'création-évaluation', details: 'Création du QCM "Algorithmes de tri"', location: 'À distance', device: 'iOS' },
  { id: 'y-3', date: new Date(new Date(yesterday).setHours(11, 20, 50)), type: 'message', details: 'Message envoyé au groupe "Projet Informatique M1"', location: 'À distance', device: 'iOS' },
  { id: 'dby-1', date: new Date(new Date(dayBeforeYesterday).setHours(8, 0, 10)), type: 'connexion', details: 'Première connexion de la journée', location: 'Campus de Paris', device: 'macOS' },
  { id: 'dby-2', date: new Date(new Date(dayBeforeYesterday).setHours(10, 30, 0)), type: 'rapport', details: 'Génération du rapport de présence pour "Bases de Données L3"', location: 'Campus de Paris', device: 'macOS' },
  { id: 'dby-3', date: new Date(new Date(dayBeforeYesterday).setHours(14, 0, 0)), type: 'publication', details: 'Publication du TD "Algorithmes de graphes"', location: 'Campus de Paris', device: 'macOS' },
];

const ITEMS_PER_PAGE = 10;

export default function ProfessorActionsPage() {
